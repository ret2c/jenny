...
Paste into agent chat: `triage <findings-path> [--auto] [--votes N] [--repo PATH] [--fp-rules FILE]`.
...
**Tools:** Use the available file/search/edit tools and ask concise questions
when interactive context is needed. Shell is limited to bounded read-only
inspection (`git`, `rg`/`grep`, `wc`, `ls`, `jq`) and
`python3 skills/_lib/checkpoint.py` for checkpoint I/O. Avoid unbounded
recursive `find`.

**Delegation and parallelism.** Delegation is optional and requires current
operator authorization. Use multi-agent tools only when that authorization is
current and the tools are available. Before delegating, read
`tools/review_mailbox/DELEGATION_TASK_PACKET_POLICY.txt`; use
`fork_turns="none"` or a small explicit positive turn count and give each
worker a bounded self-contained packet. Otherwise execute the same prompts
inline and sequentially, checkpointing after each phase or shard.
...
`python3 skills/_lib/checkpoint.py load ./.triage-state`

- `status == "absent"` OR `"complete"`, OR `--fresh` in `the user request` →
  **fresh start.** Bash:
  `python3 skills/_lib/checkpoint.py reset ./.triage-state`,
...
2. Bash → `python3 skills/_lib/checkpoint.py save ./.triage-state <N> <name> --from ./.triage-state/_chunk.tmp`

**End of run.** After writing `TRIAGE.json` and `TRIAGE.md`, Bash:
`python3 skills/_lib/checkpoint.py done ./.triage-state 6`
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.triage-state 0 interview --from ./.triage-state/_chunk.tmp`
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.triage-state 1 ingest --from ./.triage-state/_chunk.tmp`
...
Run one isolated pass with this prompt. Use a general-purpose subagent only if
multi-agent tools are available and delegation is currently operator-authorized;
otherwise run it inline without adding outside context:
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.triage-state 2 dedup --from ./.triage-state/_chunk.tmp`
...
default 3). If multi-agent tools are available and delegation is currently
operator-authorized, launch them as isolated general-purpose workers with
descriptions like `"verify {id} vote {k}/{N}"`.
...
Launch verifier workers together only when multi-agent tools are available and
delegation is currently operator-authorized. Do not background them; you need
the final text, not an async handle. If `len(candidates) * N` exceeds ~40,
shard into sequential batches of ~40. Without both tool availability and
current authorization, run the same prompts sequentially inline.
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.triage-state 3 verify --from ./.triage-state/_chunk.tmp`
...
2. Bash:
   `python3 skills/_lib/checkpoint.py shard ./.triage-state <id> --from ./.triage-state/_chunk.tmp`
...
available and delegation is currently operator-authorized, launch them
together as general-purpose workers; otherwise run them sequentially inline.
Use this prompt:
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.triage-state 4 rank --from ./.triage-state/_chunk.tmp`
...
2. **guarded Git history.** If `--repo` is a git checkout, run
   `python -B tools/guarded_git_history.py --repo {REPO} --path "{file}" --since-days 730 --max-count 50 --format authors`
   and derive the top three committers from the bounded JSON `output`. Never
   run a direct history walk on a partial/promisor checkout.
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.triage-state 5 route --from ./.triage-state/_chunk.tmp`
...
2. Bash:
   `python3 skills/_lib/checkpoint.py append ./TRIAGE.md --from ./.triage-state/_chunk.tmp`
...
## Verifying one completed triage

Use only operator-provided findings and a reachable read-only repository.
...
- **Checkpoints are per-phase JSON**, not conversation state. File-backed
  ...
  when the orchestrator's context window itself fills. `./.triage-state/` is
  ignored scratch state.
- **Dedupe runs before verify** to cut verifier spend by the duplication
  factor (often 2-4x on multi-scanner input) at the cost of one bounded
  authorized delegated pass or inline pass.
...
- **Bash is allowed narrowly** for `guarded_git_history.py` (owner hints), `jq`/`find`
