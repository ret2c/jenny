...
in `./PATCHES/` for a human to review and apply out-of-band. The reviewer checks
scope, correctness, regressions, and unintended behavior. There is no `--apply` or
...
**Tools.** Prefer the available file/search/edit tools. When dedicated search tools
...
execution is otherwise reserved for `python3 skills/_lib/checkpoint.py`
...
**Start of run.** Bash:
`python3 skills/_lib/checkpoint.py load ./.patch-state`

- `status == "absent"` OR `"complete"`, OR `--fresh` in `the user request` →
  fresh start. Bash:
  `python3 skills/_lib/checkpoint.py reset ./.patch-state`,
  proceed to Phase 0.
...
`python3 skills/_lib/checkpoint.py save ./.patch-state <N> <name> --from ./.patch-state/_chunk.tmp`

**End of run.** After writing `PATCHES.md` and `PATCHES.json`, Bash:
`python3 skills/_lib/checkpoint.py done ./.patch-state 4`
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.patch-state 0 mode --from ./.patch-state/_chunk.tmp`
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.patch-state 1 ingest --from ./.patch-state/_chunk.tmp`
...
`python3 skills/_lib/checkpoint.py shard ./.patch-state <id> --from ./.patch-state/_chunk.tmp`.
After all findings, write the consolidated phase payload to `_chunk.tmp` then:
`python3 skills/_lib/checkpoint.py save ./.patch-state 2 generate --from ./.patch-state/_chunk.tmp`
...
`{"phase": 3, "findings": [...]}`
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.patch-state 3 review --from ./.patch-state/_chunk.tmp`
...
> re-attacked. Read each diff yourself before applying; check scope,
> correctness, regressions, and unintended behavior.
...
{if static:} These are drafts. Review scope, correctness, regressions, and unintended behavior before applying.
```
