...
description: >-
  ...
  THREAT_MODEL.md if present), reviews focus areas in parallel when
  multi-agent tools are available, and writes VULN-FINDINGS.json + .md for
  triage to consume. Read-only: no building, running, or network. For
  execution-verified crashes, use vuln-pipeline instead. Use when asked to
  scan for vulns, review code for security issues, find bugs in a directory,
...
**Tool fallbacks.** Prefer the available file/search tools. When they are unavailable,
fall back to bounded read-only shell commands: `rg --files <scope>` / `ls -R`
for enumeration, `rg -n` / `grep -rn` for search, `wc` / `head` / `file` for
sniffing. Do not build, run, install dependencies, send requests, write helper
scripts, or pipe target content into an interpreter.

**Parallelism.** If multi-agent tools are available, use one subagent per
focus area to preserve the original design. If not, run the focus-area reviews
sequentially and keep the same output contract.
...
Unless `--single`, run **one review pass per focus area** in parallel when
...
`triage` see high-signal findings first. Run **one second-opinion pass per
